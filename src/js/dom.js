export const elementos = {
    primeiraPersonaNome: document.getElementById('primeiraPersonaNome'),
    innerPrimeiraPersonaNome: document.querySelector('innerPrimeiraPersonaNome'),
    primeiraPersonaSalario: document.getElementById('primeiraPersonaSalario'),

    segundaPersonaNome: document.getElementById('segundaPersonaNome'),
    innerSegundaPersonaNome: document.querySelector('innerSegundaPersonaNome'),
    segundaPersonaSalario: document.getElementById('segundaPersonaSalario'),

    infoPrimeiraPersonaNome: document.querySelector('infoPrimeiraPersonaNome'),
    infoSegundoPersonaNome: document.querySelector('infoSegundoPersonaNome'),

    valorFatura: document.getElementById("valorFatura"),

    pagarPessoaUm: document.getElementById("pagarPessoaUm"),
    porcentagemPessoaUm :document.getElementById("porcentagemPessoaUm"),
    pagarPessoaDois: document.getElementById("pagarPessoaDois"),
    porcentagemPessoaDois: document.getElementById("porcentagemPessoaDois"),

    botaoCalcular: document.getElementById("botaoCalcular"),
    botaoLimpar: document.getElementById("botaoLimpar")
}
